...
class MultiAgentNetwork(nn.Module):
    ...
    def __init__(self, state_shape, action_size, num_agents=2):
        ...
        convh = conv2d_size_out(conv2d_size_out(
            conv2d_size_out(state_shape[0])))
        convw = conv2d_size_out(conv2d_size_out(
            conv2d_size_out(state_shape[1])))
    ...
    def forward(self, states):
        ...
        batch_size = states[0].size(0) if isinstance(
            states[0], torch.Tensor) else 1

        # Process each state
        features = []
        for state in states:
            x = state.float() / 255.0 if isinstance(state,
                                                    torch.Tensor) else torch.FloatTensor(state).to(next(self.parameters()).device) / 255.0
...
class MultiAgentSystem(BaseAgent):
    ...
    def __init__(self, state_shape, action_size, num_agents=2, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        # Networks
        self.policy_net = MultiAgentNetwork(
            state_shape, action_size, num_agents).to(device)
        self.target_net = MultiAgentNetwork(
            state_shape, action_size, num_agents).to(device)
        ...
        # Move tracking per agent
        self.agent_move_trackers = [
            defaultdict(list) for _ in range(num_agents)]
        self.agent_success_rates = [
            defaultdict(float) for _ in range(num_agents)]
        # Track when agents use similar moves
    ...
    def update_networks(self):
        ...
        for agent_id, agent_batch in enumerate(agent_batches):
            ...
            actions = torch.LongTensor(
                [exp.action for exp in agent_batch]).to(self.device)
            rewards = torch.FloatTensor(
                [exp.reward for exp in agent_batch]).to(self.device)
            next_states = torch.FloatTensor(next_states).to(self.device)
            dones = torch.FloatTensor(
                [exp.done for exp in agent_batch]).to(self.device)

            # Get current Q values
            policies, values = self.policy_net([states])
            # Use first value since we process one agent at a time
            current_values = values[0].squeeze()

            # Get next values
            with torch.no_grad():
                _, next_values = self.target_net([next_states])
                target_values = rewards + \
                    (1 - dones) * self.gamma * next_values[0].squeeze()

            # Compute value loss
            value_loss = F.mse_loss(current_values, target_values)

            # Compute policy loss
            log_probs = torch.log(policies[0].gather(
    ...
    def get_agent_statistics(self, agent_id):
        ...
        stats = {
            ...
            'success_rates': {
                move_type: success_rates[move_type] /
                len(moves) if len(moves) > 0 else 0
                for move_type, moves in move_tracker.items()
            }
        }
        return stats
    ...
    def load(self, path):
        ...
        self.agent_interactions = defaultdict(
            int, checkpoint['agent_interactions'])
